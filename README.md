...
macOS のメニューバーに常駐し、範囲選択したスクリーンショットを JAPAN AI LLM Gateway で OCR してクリップボードにコピーするツール。
...
- OCR 中はトレイアイコンのタイトルが「OCRing」になり、完了/失敗は macOS 通知で知らされる
...
- OCR は OpenAI 互換の `/v1/chat/completions` を使用するため、選択するモデルは画像入力に対応している必要がある
- OCR のレイテンシはモデル依存で、数秒〜10秒を超えることがある（HTTP タイムアウトは120秒に設定）
- LLM Gateway の JSON リクエスト上限を超えないよう、キャプチャ画像は11 MiB以下に制限される
- 本ツールはキャプチャ画像を PNG のData URLに変換して送信する
